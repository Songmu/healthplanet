...
BUILD_LDFLAGS = "-s -w -X github.com/Songmu/healthplanet.revision=$(CURRENT_REVISION)"
u := $(if $(update),-u)
...
.PHONY: build
build:
	go build -ldflags=$(BUILD_LDFLAGS) ./cmd/healthplanet

.PHONY: install
install:
	go install -ldflags=$(BUILD_LDFLAGS) ./cmd/healthplanet
